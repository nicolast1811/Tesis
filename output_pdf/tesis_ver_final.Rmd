...
lang: es
csl: archivos/apa.csl
bibliography: archivos/referencias.bib
header-includes:
  - \usepackage{fontspec}
  - \setmainfont{Arial}
  - \usepackage{tocloft}
  ...
  # Prefijos con dos puntos
  - \renewcommand{\cftfigpresnum}{Figura~}
  - \renewcommand{\cftfigaftersnum}{:\quad}
  - \renewcommand{\cfttabpresnum}{Tabla~}
  - \renewcommand{\cfttabaftersnum}{:\quad}

  # Ajuste de espacio para que los títulos largos se partan bien
...
\input{archivos/portada.tex}
\newpage
\input{archivos/hoja_cc.tex}
\newpage
\clearpage
\newpage
...
De igual manera, agradezco la oportunidad de haber cursado el *minor* en Ciencia de Datos de la Facultad de Ingeniería, que me entregó una mirada distinta del mundo y donde pude aprender muchos conceptos y técnicas de *software* que fueron aplicados en este trabajo.
...
```{r setup-graphs, include=FALSE}
# Tamaño de los gráficos 
knitr::opts_chunk$set(
  fig.width = 8,      # Ancho de los gráficos (en pulgadas)
  fig.height = 6,     # Alto de los gráficos (en pulgadas)
  out.width = "80%",  # Anchura de los gráficos en HTML/PDF
  out.height = if (knitr::is_latex_output()) "6in" else "400px" # Tamaño ajustable
)
```

```{r librerias, include=FALSE}
# Lista de paquetes necesarios
paquetes_necesarios <- paquetes <- c(
...
# Ejecutar la función con la lista de paquetes
instalar_y_cargar(paquetes_necesarios)
# Para que los gráficos salgan con la fuente y tamaño correcto
showtext_auto(enable = TRUE) 
```

```{r carga-datos, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
# Esto fijar here() en la carpeta donde está data.RData
here::i_am("data.RData")

# Raíz de here(), muy útil para las cargas automatizadas de BBDD 
here()
# Al cargar el archivo data.RData se cargaran a la sesión todos los objetos
# necesarios para reproducir el código en su totalidad

# Carga de bases de datos, gráficos y tablas
load(here("data.RData"))

# Si no funciona, cargar manualmente cada script
# source(here("scripts", "data.R"), encoding = "UTF-8")
# source(here("scripts", "graficos.R"), encoding = "UTF-8")
# source(here("scripts", "tablas.R"), encoding = "UTF-8")
```
...
**Palabras clave:** *Servicios Locales de Educación Pública (SLEP), análisis cuantitativo, K-Means, PCA, vulnerabilidad socioeconómica, equidad educativa*
...
Este modelo, basado en la descentralización y una lógica de mercado, ha sido criticado por su incapacidad para reducir las desigualdades educativas y asegurar una calidad homogénea en el sistema educativo, generando profundas brechas entre comunas y regiones de diferentes niveles socioeconómicos [@baleriola_traduccion_2021].

En respuesta a estas limitaciones, la Ley N.º 21.040, promulgada en 2017, marca un cambio de paradigma en la gestión de la educación pública chilena al establecer los nuevos paradigmas de la educación pública al crear los Servicios Locales de Educación Pública [@biblioteca_del_congreso_nacional_de_chile_ley_2017].
...
Este modelo tiene como objetivo profesionalizar la administración, optimizar el uso de los recursos y garantizar que todos los estudiantes, independientemente de su ubicación geográfica o condición socioeconómica, tengan acceso a una educación de calidad [@bellei_nueva_2018].
...
Con este primer gran hito, se consolida la educación pública como "centralizada, altamente burocrática y jerárquica" [@bellei_nueva_2018, p.22], conceptos propios de organizaciones de inicios del siglo pasado, pero que a pesar de todo dieron frutos.
...
Un claro ejemplo de esto fueron los profesores normalistas, quienes fueron los primeros profesores con conocimientos avanzados y superiores de materias y pedagogía, siendo así pilares fundamentales del sistema educativo de la época, y sentaron las bases para la enseñanza en Chile [@memoriachilena]

## Transformaciones en el sistema educativo chileno
...
Esto se demostró en la "revolución pingüina", durante 2006 [@bellei_ecos_2010, pp. 12-13].
...
El sistema educativo chileno tenía evidentes problemas, que requieren solución [@bellei_nueva_2018, p. 37].
...
## Nacimiento de la Nueva Educación Pública y los SLEP

Como se ha mencionado, la ley sobre los SLEP y NEP fue promulgada en 2017, y empezó a aplicarse desde el año siguiente.
Es una medida reciente, y el proceso denominado como "desmunicipalización", esto es, traspasar las facultades administrativas de las municipales o corporaciones de educación hacia el nuevo organismo de nivel intermedio llamado SLEP, todavía se está llevando a cabo [@biblioteca_del_congreso_nacional_de_chile_ley_2017].
...
Estos se suman a los 15 que ya estaban en funcionamiento, teniendo un total de 24 SLEP operativos en todo el país [@ministerio_de_educacion_mapa_2024].
...
A su vez, existe una visión "heroica" de los trabajadores y sobre todo de los profesores, que sienten que su deber frente a los niños es más grande que sus condiciones laborales que muchas veces no son las adecuadas [@garreton_brechas_2022, pp. 11-15].
...
1)  Gestión Administrativa: los encuestados comentan que la burocracia que empezó a suscitar con el traspaso de municipalidad a SLEP dificulta el accionar rápido que se requiere cuando se trabaja en educación primaria y secundaria;

2)  Participación y comunicación: existe poca o nula vinculación con el medio de los SLEP, es decir, la comunicación entre el Servicio y las escuelas es difícil de realizar;

3)  Convivencia escolar: se requiere de un trabajo más profundo y directo en los territorios, y finalmente

4)  Monitoreo de aprendizajes: falta de claridad en cómo evaluar el currículo escolar y retroalimentación de los procesos de enseñanza y aprendizaje.
...
El informe de @fundacion_horizonte_ciudadano_herramientas_2024 contó con gran participación de distintas partes involucradas en la implementación de la ley, especialmente los referidos a municipalidades, que como hemos mencionado en este apartado, son quienes están traspasando facultades hacia los nuevos Servicios Locales.

Su metodología fueron conversatorios con actores del sector municipal, talleres con académicos y exautoridades, cuestionarios a funcionarios de los SLEP y también revisión de propuestas y documentos varios que han surgido para la regularización de los SLEP [@fundacion_horizonte_ciudadano_herramientas_2024, p. 3].

Entre sus principales hallazgos, se encuentran cinco "nudos críticos" del traspaso del municipio a la nueva entidad:

1.  Gestión de recursos y dotaciones: los municipios argumentan que han tenido problemas para una gestión presupuestaria de los recursos asociados al traspaso, tanto financieros como humanos;

2.  Infraestructura y equipamiento escolar: el traspaso no ha venido acompañado, necesariamente, de mejoras estructurales a los establecimientos, que son problemas que se arrastrarán hasta los nuevos administradores de los establecimientos;

3.  Liderazgo directivo y colaboración: los directores y altos mandos de los establecimientos siguen teniendo responsabilidades administrativas, y aún existe un alto porcentaje de directores que no han pasado por el Sistema de Alta Dirección Pública;

4.  Confianza y creación de sentido sobre la NEP: los problemas en la implementación temprana de los traspasos, agravados muchas veces por la manera de presentar los problemas en la opinión pública, crean desconfianza en la población hacia los nuevos SLEP, también se menciona la resistencia al cambio, y finalmente;

5.  Calidad integral de la educación: no solamente se menciona la educación un proceso de clases lectivas donde existe un traspaso de conocimientos desde el docente al alumno, sino que también se requiere un entendimiento de esta como una instancia para el desarrollo de las personas, aquí se mencionan factores como la interdependencia entre la escuela, comunidad y familias.
...
## Reflexión y justificación de investigación
...
## La educación y su relación con el capital humano
...
@locatelli_educacion_2018 menciona al respecto que:
...
> [@locatelli_educacion_2018, p. 180]
...
> [@locatelli_educacion_2018, p. 191]
...
## La Nueva Educación Pública y la transferencia de funciones administrativas
...
En palabras de @mardones_z_descentralizacion_2008, el traspaso de funciones desde un nivel superior a subnacional puede ocurrir de tres maneras distintas:

1.  Desconcentración: donde el nivel superior traspasa hacia abajo, al interior de una estructura piramidal y burocrática, atribuciones
...
3.  Descentralización: que implica la entrega de funciones a gobiernos subnacionales autónomos, y donde la principal tarea de diseñar, monitorear, financiar, implementar, supervisar, entre otras, depende mayoritariamente de los gobiernos locales.
...
Con respecto a su implementación, durante la última década se ha criticado bastante su desempeño, y principalmente por su "lógica de prácticas administrativas del sector privado, así como de un intenso discurso a favor del libre mercado" [@montemayor_tendencias_2018, p. 34].

En lo referido al control presupuestario hacia el Estado, Lorenz (2012, citado en @montemayor_tendencias_2018) argumenta que la disminución de recursos para el sector público afecta la calidad y cantidad de servicios gubernamentales, lo que genera mayor gasto de los ciudadanos para solventarlos, así como también ha desfavorecido la profesionalización de los administradores de este.
...
Es un cambio de paradigma respecto a la educación pública en Chile que pretende ser una "reversión de las políticas públicas educativas" heredadas de la dictadura [@baleriola_traduccion_2021, p. 3].
...
## La ciencia de datos y la toma de decisiones
...
Según @sarker_data_2021, la ciencia de datos es un esfuerzo para unir las estadísticas, el análisis de datos y sus métodos para entender y analizar el fenómeno en base a qué dicen los datos.
...
1.  Análisis descriptivo: que responden a la pregunta "¿Qué pasó?";

2.  Análisis diagnóstico: que responden a la pregunta "¿Por qué pasó?"

3.  Análisis predictivo: que se esfuerza por dar una respuesta a lo que podrá ocurrir, y finalmente;

4.  Análisis prescriptivo: que analiza que se acción se debería tomar.
    [@sarker_data_2021, p. 2]
...
> [...] la minería de datos surge como una posible respuesta, a la necesidad de análisis, manipulación y extracción de conocimiento de los datos, toda vez que ésta hace uso de sofisticados algoritmos que permiten encontrar patrones ocultos en un conjunto de datos y predecir comportamientos.
> [@garcia-gonzalez_knowledge_2019_a], p. 56]
...
1.  Identificar patrones entre los SLEP seleccionados mediante la creación de clústers basados en características como matrícula, ruralidad, resultados en pruebas estandarizadas y diferencias socioeconómicas de los alumnos que componen los establecimientos.

2.  Examinar tales patrones para descubrir de qué manera se configuraron los establecimientos luego del traspaso de funciones a los Servicios Locales.

3.  Encontrar similitudes y diferencias entre estos patrones para dar una caracterización de los establecimientos pertenecientes a cada grupo, y explicar sus características para la toma de decisiones basada en datos.
...
*H1: La implementación de la Ley 21.040 ha generado cambios profundos en los establecimientos educativos de los SLEP seleccionados, en las dimensiones a trabajar.*

*H2: Se pueden trazar similitudes y diferencias en los SLEP seleccionados en los ámbitos estudiados.*

*H3: Los SLEP seleccionados con alta ruralidad enfrentan mayores desafíos en términos de gestión, lo que se manifiesta en menores recursos humanos y desempeño académico.*
...
```{r tabla-slep, results="asis", echo=FALSE}
library(kableExtra)
# Generar tabla optimizada en formato LaTeX
library(kableExtra)
...
tabla_muestra_slep %>%
  select(
    ...
    Región,
    Comunas,
    `Año Inicio de Funciones`,
    `Nº Establecimientos`,
    `% del Total` = Porcentaje,
    Matrícula
  ) %>%
    mutate(
    `% del Total` = format(`% del Total`, decimal.mark = ","),
    Matrícula = format(Matrícula, big.mark = ".", decimal.mark = ",")
  ) %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "Características generales de los SLEP analizados",
  ...
  column_spec(1, width = "3.5cm", bold= TRUE) %>%  # Nombre SLEP
  column_spec(2, width = "2.2cm") %>%  # Región
  column_spec(3, width = "4.7cm") %>%  # Comunas (ligera reducción)
  column_spec(4, width = "2.3cm") %>%  # Año de Inicio
  column_spec(5, width = "2.4cm") %>%  # Nº Establecimientos (ligeramente más ancho)
  column_spec(6, width = "2.4cm") %>%  # % del Total (ligeramente más ancho)
  column_spec(7, width = "2.3cm") %>%  # Matrícula (ligera reducción)
  footnote(
    general = "Fuente: Elaboración propia en base a datos MINEDUC y ACE.",
    general_title = ""
  )
```
...
A su vez, tenemos tanto comunas con baja ruralidad, como el Gabriela mistral que a 2023 no registraba ruralidad, o el de Barrancas, con un 5,6%, hasta Servicios Locales con cerca de un 70% de ruralidad, como el Costa Araucanía [@berkowitz_servicios_2024, p. 33].
...
En esta misma línea, @hernandez-sampieri_metodologiinvestigacion_2018 , en los capítulos tercero y cuarto de su libro, ofrece una metodología sólida que ha sido clave para definir la muestra, las preguntas, los objetivos y las hipótesis de esta investigación.
...
También se encuentran recomendaciones para la temporalidad de la investigación y la factibilidad de tiempos, que fueron tomadas en cuenta para la redacción de este documento [@hernandez-sampieri_metodologiinvestigacion_2018, pp. 40-44].
...
Estas fuentes proporcionan datos actualizados y estandarizados sobre variables clave del sistema educativo, como matrícula, resultados SIMCE, dotación docente, nivel de vulnerabilidad, entre otras [@centro_de_estudios_mineduc_datos_nodate; @agencia_de_la_calidad_de_la_educacion_bases_nodate].
...
Con respecto a la estrategia metodológica, se usará la metodología conocida como *knowledge discovery in databases (KDD)*, lo que en español sería "descubrimiento de conocimiento en bases de datos".
...
```{r tabla-kdd, results="asis", echo=FALSE}
# Cargar librerías necesarias
library(kableExtra)

# Generar tabla optimizada en formato LaTeX
kable(tabla_KDD, format = "latex", booktabs = TRUE, 
      caption = "Proceso de Knowledge Discovery in Databases (KDD)",
      align = c("c", "l")) %>%  # Centra "Paso" y justifica "Descripción"
  kable_styling(latex_options = c("hold_position", "striped", "scale_down"), full_width = FALSE) %>%
  column_spec(1, width = "4cm", bold = TRUE) %>%  # Ajustar ancho de "Paso"
  column_spec(2, width = "10cm") %>%  # Ampliar la columna "Descripción" para mejor lectura
  footnote(general = "Fuente: Elaboración propia con base en Fayyad et al. (1996).", general_title = "")
...
> "the KDD process involves using the database along with any required selection, preprocessing, subsampling, and transformations of it; applying data-mining methods (algorithms) to enumerate patterns from it; and evaluating the products of data mining to identify the subset of the enumerated patterns deemed knowledge." [el proceso KDD implica usar la base de datos original junto con cualquier tipo de selección, preprocesamiento, muestreo y transformación; aplicar métodos de minería de datos (algoritmos) para encontrar patrones de ella, y, evaluarlos para identificar la submuestra de patrones creados que aporta conocimiento].[@fayyad_data_1996, p. 42]
...
Este método consiste en generar, a partir del conjunto total de datos, conglomerados (también conocidos como *clústers*) que contienen características similares entre ellos, y que a su vez son distintos a otros grupos que se formen.
...
Su principal objetivo es encontrar centroides dentro del conjunto de datos ya graficado para generar los *clústers*, es decir, el centroide es la idea de lo que es un elemento promedio de cada grupo.
De esta manera, se agrupan las observaciones -en este caso establecimientos- con relación a qué tan cerca está de un centroide y que tan alejado está de otro [@umargono_k-means_2020, p. 121-122].
...
Este fenómeno se conoce como la maldición de la dimensionalidad [@aurelien_geron_hands-machine_2017, pp. 208-209].
A medida que quiero estudiar más características de determinado fenómeno, se vuelve imposible graficarlo.

Existen varias técnicas de machine learning y manejo de datos para lidiar con esto, y la más utilizada y que da mejores resultados es el análisis de componentes principales (PCA por sus siglas en inglés), que se encarga de reducir al máximo las dimensiones que usaremos, sin perder tanta información [@ringner_what_2008].
En palabras simples, PCA se encarga de reducir la cantidad de dimensiones, sin perder su información.
Esto quiere decir que en los primeros componentes se explican la mayoría de los datos, y de esta manera datos de muchas dimensiones se pueden graficar en 2 o 3 dimensiones, para hacer más comprensible su comportamiento [@aurelien_geron_hands-machine_2017, pp. 213-219].
...
## Principales características de los SLEP seleccionados
...
```{r grafico-simce-l, echo=FALSE, fig.cap="Evolución de puntajes SIMCE de Lectura (2017–2023)", fig.align='center', warning=FALSE}
g_simce_l
```
...
En particular, en los años 2020 y 2021 se cambió esta evaluación por un diagnóstico integral, voluntario de cada establecimiento, con motivo de la pandemia y el confinamiento que regía en el país [@diario_uchile_pruebe_2020].
...
```{r grafico-simce-m, echo=FALSE, fig.cap="Evolución de puntajes SIMCE de Matemática (2017–2023)", fig.align='center', warning=FALSE}
g_simce_m
```
...
```{r grafico-ruralidad, echo=FALSE, fig.cap="Número de establecimientos rurales y urbanos", fig.align='center', warning=FALSE}
g_ruralidad
```
...
```{r tabla-ruralidad, results="asis", echo=FALSE}
# Cargar librerías necesarias
library(kableExtra)

# Generar tabla optimizada en formato LaTeX
kable(ruralidad_slep, format = "latex", booktabs = TRUE, 
      caption = "Establecimientos rurales y urbanos por SLEP",
      align = c("l", "c", "c", "c", "c", "c"),
      col.names = c("SLEP", "Urbano", "% Urbano", "Rural", "% Rural", "Total")) %>%  # Cambiar encabezado de última columna
  kable_styling(latex_options = c("hold_position", "striped", "scale_down"), full_width = FALSE) %>%
  column_spec(1, width = "3.8cm", bold = TRUE) %>%
  column_spec(2:5, width = "2.1cm") %>%  # Reducir un poco el ancho de estas columnas
  column_spec(6, width = "2.5cm") %>%  # Acomodar columna Total
  footnote(general = "Fuente: Elaboración propia en base a datos MINEDUC y ACE.", general_title = "")
```
...
```{r grafico-matricula, echo=FALSE, fig.cap="Evolución de matrícula por SLEP", fig.align='center', warning=FALSE}
g_matricula
# \@ref(fig:grafico-matricula)
```
...
```{r grafico-genero, echo=FALSE, fig.cap="Proporción de género por docentes SLEP", fig.align='center', warning=FALSE}
g_genero_docente
# \@ref(fig:grafico-matricula)
```

```{r grafico-genero-horas, echo=FALSE, fig.cap="Promedio de horas trabajadas por género", fig.align='center', warning=FALSE}
g_genero_horas_docente
# \@ref(fig:grafico-matricula)
```
...
Revisada la página web del MINEDUC, los para docentes se entienden de manera bastante amplia, ya que pueden ser desde administrativos hasta auxiliares de aseo [@ministerio_de_educacion_asistentes_nodate].

```{r grafico-asistentes-jorn, echo=FALSE, fig.cap="Carga horaria por estamento", fig.align='center', warning=FALSE}
g_horas_asis
# \@ref(fig:grafico-matricula)
```

## Reducción de dimensionalidad con PCA y agrupamiento con K-Means
...
```{r grafico-var-acum, echo=FALSE, fig.cap="Varianza explicada acumulada en análisis de componentes principales (PCA)", fig.align='center', warning=FALSE}
g_var_expl
# \@ref(fig:grafico-matricula)
```
...
Ahora, viene la parte de elegir el número de centroides en los que se separará nuestro conjunto de datos, y para ello se usó, siguiendo la bibliografía consultada, el método del codo explicado en @umargono_k-means_2020 y también revisado en @gutierrez_k-means_2021.
...
En su estudio, Umargono et al. demostraron que aplicar esta técnica no solo mejora la representatividad de los clústers obtenidos, sino que además reduce significativamente el número de iteraciones requeridas por el algoritmo, optimizando así el proceso de agrupamiento [@umargono_k-means_2020, pp. 127-129].
...
```{r grafico-codo, echo=FALSE, fig.cap="Método del codo para determinar número óptimo de clústers (K)", fig.align='center', warning=FALSE}
g_codo
# \@ref(fig:grafico-matricula)
```

En un proceso iterativo, se decidió entrenar distintas instancias de K-Means (de 2 a 10 centroides) para evaluar el desempeño de los *clústers.* Para ello, se usó la métrica de el puntaje de silueta (*silhouette score* en inglés).

Esta métrica analiza las distancias entre los *clústers* generados para ver qué tan similares son intra-grupo y que tan diferentes son extra-grupo.
Su valor va desde -1 a 1, donde valores cercanos a -1 nos indica que el elemento está en el grupo equivocado, 0 nos dice que podría estar en cualquier otro (nuestro algoritmo no detectó diferencias o similitudes) y valores cercanos a 1 nos indica que el elemento está en el grupo correcto [@shahapure_cluster_2020].
...
```{r tabla-sil, results="asis", echo=FALSE}
# Cargar librerías necesarias
library(kableExtra)
library(dplyr)

# Generar tabla optimizada en formato LaTeX
kable(tabla_silueta, format = "latex", booktabs = TRUE, 
      caption = "Puntaje de silueta según número de clústers (K = 2 a 10)",
      align = c("c", "c")) %>%  # Alinear ambas columnas al centro
  kable_styling(latex_options = c("hold_position", "striped", "scale_down"), full_width = FALSE) %>%
  column_spec(1, width = "3cm", bold = TRUE) %>%  # Ajustar ancho y negrita para Número de clústers
  column_spec(2, width = "3cm")  # Ajustar ancho de Puntaje
...
```{r grafico-cluster, echo=FALSE, fig.cap="Distribución de clústers identificados mediante K-Means y PCA", fig.align='center', warning=FALSE}
g_cluster
# \@ref(fig:grafico-matricula)
```
...
```{r grafico-detalle-cluster, echo=FALSE, fig.cap="Cantidad de SLEP por clúster", fig.align='center', warning=FALSE}
g_n_cluster
# \@ref(fig:grafico-matricula)
```
...
```{r grafico-caract-cluster, echo=FALSE, fig.cap="Mapa de calor de variables normalizadas por clúster", fig.align='center', warning=FALSE}
g_heatmap
# \@ref(fig:grafico-matricula)
```
...
## Capital humano y datos generales de establecimientos
...
## El traspaso de funciones administrativas y su vinculación con los datos estudiados
...
## La ciencia de datos y la toma de decisiones basada en datos
...
**Grupo 1: establecimientos intermedios y equilibrados**
...
**Grupo 2: establecimientos de alta vulnerabilidad socioeconómica y ruralidad**
...
**Grupo 3: establecimientos con alto rendimiento académico y menor vulnerabilidad**
...
```{r tabla-cluster-resumido, results="asis", echo=FALSE}
library(kableExtra)
library(dplyr)

# Generar tabla optimizada en formato LaTeX
kable(caracterizacion_clusters_resumen, format = "latex", booktabs = TRUE, 
      caption = "Características resumidas de cada grupo",
      align = c("c", "l")) %>%  # Especificar alineación en kable()
  kable_styling(latex_options = c("hold_position", "striped", "scale_down"), full_width = FALSE) %>%
  column_spec(1, width = "2cm", bold = TRUE) %>%  # Ajustar ancho de la columna de clústers y negrita
  column_spec(2, width = "10cm")  # Ampliar la columna de Descripción
...
1.  Evaluaciones que sigan la trayectoria de los SLEP en el tiempo y midan su impacto sostenido en los resultados educativos.

2.  Estudios cualitativos territoriales que recojan percepciones de actores locales, como docentes y directivos, respecto a los cambios administrativos y pedagógicos a medida que más establecimientos den cumplimiento al traspaso dispuesto en la ley.

3.  Análisis comparativos internacionales que permitan situar el caso chileno dentro de experiencias similares de transferencia de funciones en la administración de los establecimientos de la educación pública.
...
## Anexo A. Tabla de resumen de las 29 variables analizadas

```{r anexo-a, results="asis", echo=FALSE}
kable(tabla_etiquetas, format = "latex", booktabs = TRUE) %>%
  kable_styling(
    latex_options = c("hold_position", "striped", "scale_down"),
    full_width = FALSE,
    font_size = 10
...
## Anexo B. Contribuciones por variable a los dos primeros componentes principales

```{r anexo-b, results="asis", echo=FALSE}
g_contribs
```

## Anexo C. Ponderación de variables por clúster

```{r anexo-c, results="asis", echo=FALSE}
g_pesos_cluster
```

\newpage

## Anexo D. Gráfico de puntaje de silueta por clúster

```{r anexo-d, results="asis", echo=FALSE}
g_silueta_score
```
